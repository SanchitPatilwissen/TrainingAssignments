...
class Menu {
    ...
    static void InputData(int num2) {
        ...
        EmployeeCreation(num2, id, name, age, salary, department);
    }

    static final void EmployeeCreation(int num2, int id, String name, int age, double salary, int department) {
        String[] department_map = {"SALES", "FINANCE", "HR", "IT"};
        String[] designation_map = {"CLERK", "PROGRAMMAR", "MANAGER", "OTHER"};

        try {            
            Database.createEmployee(id, name, age, salary, designation_map[num2-1], department_map[department-1]);
        } catch (Exception e) {
